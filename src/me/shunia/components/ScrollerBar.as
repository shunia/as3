package me.shunia.components
{
	...
	public class ScrollerBar extends Sprite
	{
		...
		protected function updateSize():void {
			if (_props.hasPorp(P_VIEW_SIZE)) _viewSize = _props.safeProp(P_VIEW_SIZE, _viewSize);
			if (_props.hasPorp(P_CONTENT_SIZE)) _contentSize = _props.safeProp(P_CONTENT_SIZE, _contentSize);
		}
		...
		protected function calculateAndApply():void {
			...
			// 是否显示此控件
			if (_props.hasPorp(P_ALWAYS_SHOW)) _show = _props.safeProp(P_ALWAYS_SHOW, _show);
		...
		protected function onVerticalCalculation():void {
			...
			_thumbSize.setEmpty();
			_thumbSize.width = _thumb.width;
			_scale = _contentSize.height / _viewSize.height;
			_thumbSize.height = _viewSize.height / _contentSize.height * _viewSize.height;
		...
		protected function onHorizontalCalculation():void {
			...
			_scale = _contentSize.width / _viewSize.width;
			_thumbSize.width = _viewSize.width / _contentSize.width * _viewSize.width;
		...
		protected function onEvent():void {
			...
			_drag.start();
			if (_props.hasPorp(P_ON_UPDATE)) _cb = _props.safeProp(P_ON_UPDATE, _cb);
		}
		...
		protected function onScroll(dx:Number, dy:Number):void {
			...
			if (_cb != null) _cb.apply(this, [dx * _scale, dy * _scale]);
		}
