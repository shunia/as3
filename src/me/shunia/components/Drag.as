package me.shunia.components
{
	...
	public class Drag
	{
		...
		protected var _targetBound:Rectangle = null;
		...
		protected function over(e:MouseEvent):void {
			...
			try {
				...
				debug && trace("No supported mouse cursor [drag]!");
			};
		}
		...
		protected function down(e:MouseEvent):void {
			...
			if (!_lockBound) _lockBound = _target.parent.getBounds(_target.parent);
			_targetBound = _target.getBounds(_target.parent);
			_ox = e.stageX;
			_oy = e.stageY;
...
//			e.stopPropagation();
			...
			_ox = e.stageX;
			_oy = e.stageY;
			debug && trace("Before: " + dx + "," + dy);
			var p:Point = restrict(target, dx, dy, _lockBound);
			debug && trace("After: " + p.x + "," + p.y);
